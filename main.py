...
def read_wholelife_plan(file_path):
    """
    ...
    for info in data:
        ...
        if key not in wholelife_plan:
            wholelife_plan[key] = []
        wholelife_plan[key].append(info[5])
    return wholelife_plan
...
def Year_Plan(config):
    ...
    # 读取全寿命计划表
    wholelife_plan = read_wholelife_plan('./results/全寿命计划.xlsx')
    wholelife = []
    year = []
    for info in work_package:
