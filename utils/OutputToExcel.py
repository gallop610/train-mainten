...
def output_wholeLife_plan(workpackage):
    ...
    for i in range(len(workpackage)):
        ...
        for j in range(len(work.mainten_quarter)):
            ...
            # 红线欠修
            red_line_repair_date = add_quarters(last_mainten,int(math.ceil(work.Work_Package_Interval_Conversion_Value*0.90/90)))
            # 红线过修
            red_line_over_date = add_quarters(last_mainten,int(math.ceil(work.Work_Package_Interval_Conversion_Value*1.05/90)))
            track_type = ''
            tmp = [work.Train_Type,work.Train_Number,work.Work_Package_Number,work.Work_Package_Name,work.Electrifiable,work.Powerless,work.Work_Package_Interval_Time_Dimension,
                   ...
                   work.Work_Package_Contract,work.Undercarriage_Assembly,date,last_mainten,interval_quarter,over_under_repaired,over_under_repaired_percentage,red_line_repair_date,red_line_over_date,track_type]
    ...
    for i in range(len(workpackage)):
        ...
        for j in range(len(work.mainten_month)):
            ...
            if work.Work_Package_Interval_Conversion_Value>90:
                red_line_repair_date = add_months(last_mainten,int(math.ceil(work.Work_Package_Interval_Conversion_Value*0.90/90)))
            else:
                red_line_repair_date = add_months(last_mainten,int(math.ceil(work.Work_Package_Interval_Conversion_Value*0.95/30)))
            # 红线过修
            red_line_over_date = add_months(last_mainten,int(math.ceil(work.Work_Package_Interval_Conversion_Value*1.05/90)))
            track_type = ''                
...
def output_month_plan(workpackage):
    ...
    for i in range(len(workpackage)):
        ...
        for j in range(len(work.mainten_day)):
            ...
            # 红线欠修
            if work.Work_Package_Interval_Conversion_Value>90:
                red_line_repair_date = last_mainten + relativedelta(days=int(work.Work_Package_Interval_Conversion_Value*0.90))
            else:
                red_line_repair_date = last_mainten + relativedelta(days=int(work.Work_Package_Interval_Conversion_Value*0.95))
            # 红线过修
            red_line_over_date = last_mainten + relativedelta(days=int(work.Work_Package_Interval_Conversion_Value*1.05))
            track_type = ''                
                
            tmp = [work.Train_Type,work.Train_Number,work.Work_Package_Number,work.Work_Package_Name,work.Electrifiable,work.Powerless,work.Work_Package_Interval_Time_Dimension,
                   work.Work_Package_Interval_Value,work.Work_Package_Interval_Conversion_Value,work.Work_Package_Person_Day,work.Work_Package_Person_Day_Unit_Price,work.Need_Trial_Run,
                   work.Work_On_Non_Working_Day,work.Cooling_Time,work.Shared_Cooling_Work_Package_Number,work.Sampling_Times,work.Combined_Work_Package_Number,work.Need_High_Voltage_Verification,
                   work.Work_Package_Contract,work.Undercarriage_Assembly,date.strftime("%Y-%m-%d"),last_mainten.strftime("%Y-%m-%d"),interval_quarter,over_under_repaired,over_under_repaired_percentage,red_line_repair_date.strftime("%Y-%m-%d"),red_line_over_date.strftime("%Y-%m-%d"),track_type]    
